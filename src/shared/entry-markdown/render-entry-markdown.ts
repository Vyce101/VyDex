// Renders validated Entry Markdown as escaped semantic HTML.
import type { Nodes, Parent, Root } from "mdast";
import remarkGfm from "remark-gfm";
import remarkParse from "remark-parse";
import { unified } from "unified";

const entryMarkdownParser = unified().use(remarkParse).use(remarkGfm);
...
function renderLink(
  ...
  if (!isSafeLink(url)) throw new Error("Entry Markdown contains an unsafe link.");
...
function renderTable(node: Extract<Nodes, { type: "table" }>, definitions: DefinitionLookup): string {
  ...
  const body = rows.length > 0 ? `<tbody>${rows.map((row) => renderRow(row, "td")).join("")}</tbody>` : "";
...
function renderNode(node: Nodes, definitions: DefinitionLookup): string {
  switch (node.type) {
    ...
    case "linkReference": {
      ...
      if (!definition) throw new Error(`Entry Markdown has no definition for ${node.identifier}.`);
    ...
    case "tableRow":
    case "tableCell":
      throw new Error(`Entry Markdown contains an unexpected standalone ${node.type}.`);
    case "code": {
      const language = node.lang ? ` class="language-${escapeHtml(node.lang)}"` : "";
      return `<pre><code${language}>${escapeHtml(node.value)}</code></pre>`;
    }
    default:
      throw new Error(`Entry Markdown contains unsupported Markdown: ${node.type}.`);
  }
}

function parseEntryMarkdown(value: string): Root {
  try {
    return entryMarkdownParser.parse(value) as Root;
  } catch {
    throw new Error("Entry Markdown must be valid Markdown.");
  }
}
...
export function renderEntryInlineMarkdown(value: string): string {
  const tree = parseEntryMarkdown(value);
  if (tree.children.length !== 1 || tree.children[0]?.type !== "paragraph") {
    throw new Error("Entry inline Markdown requires one paragraph.");
  }
