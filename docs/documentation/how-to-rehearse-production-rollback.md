...
Use this guide after the first complete Stage 1 production deployment succeeds or when the deployment or rollback mechanism changes. The protected workflow temporarily switches the live Pages deployment, verifies it, restores the intended deployment, and verifies production again.
...
The rehearsal changes the live production deployment twice. Run it only for launch acceptance or when an explicit later rehearsal is needed; it is not part of every routine content deployment.
...
1. Confirm that the `rehearse` job succeeded.
