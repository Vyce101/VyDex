...
- Static generation of current public Entry routes and the generic `404.html` through thin Astro pages.
...
The `/` route is the static [Stage 1 Homepage](stage-1-homepage.md). Current public Entry routes are generated by the [Stage 1 Entry Page](stage-1-entry-page.md), and the project-level `404.astro` produces the generic static `404.html`. Their complete release-selected content is generated into HTML.
...
The [Stage 1 Homepage](stage-1-homepage.md) and [Stage 1 Entry Page](stage-1-entry-page.md) receive resolved release data through the Astro boundary. Their feature modules do not load files, access the environment, generate release metadata, or fetch Entry data in the browser.
...
- `src/shared/entry-markdown/` — Presentation-safe Entry Markdown shared by preview and full-page features.
