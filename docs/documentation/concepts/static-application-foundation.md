...
- Cloudflare project settings, production deployment, or rollback behavior. [Cloudflare Pages Deployment](cloudflare-pages-deployment.md) owns that system.
...
5. `npm run build:test` type-checks the project, runs Vitest, and generates deterministic static output with fixed non-production release metadata. Ordinary Playwright validation uses this explicit test mode.
6. `npm run build` performs the same checks but loads the persisted production descriptor and requires a valid production origin.
7. `npm run release:stage-1` is the bootstrap-capable atomic production gate. `npm run release:stage-1:ci` requires committed descriptor and manifest state and verifies exact reproduction without creating identity. Both build into isolated staging, run browser checks against that output, and promote locally; neither deploys.
...
[Cloudflare Pages Deployment](cloudflare-pages-deployment.md) consumes the foundation's static output after strict release validation. The deployment workflow does not move Cloudflare concerns into Astro pages or domain modules.

[Hosted Release Verification](hosted-release-verification.md) reuses the same desktop and mobile projects, keyboard and focus checks, supported-width overflow assertions, downloads, no-JavaScript journeys, and Axe scans against the live origin. Hosting API access remains outside the browser harness.
...
- The repository has a complete validated Stage 1 seed record set and committed initial release state. Development, unit tests, browser tests, and conformance builds use one fixed non-production descriptor value without writing it. Production builds never fall back to that adapter.
...
- `scripts/deployment/` — Pages preview preparation and production artifact preflight.
...
- Whether a hosting or artifact-transfer change belongs to [Cloudflare Pages Deployment](cloudflare-pages-deployment.md).
- Whether hosted browser behavior belongs to [Hosted Release Verification](hosted-release-verification.md) rather than the ordinary local test command.
