...
7. `npm run release:stage-1` runs the atomic production gate with the approved Stage 1 origin. It builds into isolated staging and promotes verified output; it does not deploy.
8. `npm run test:browser` rebuilds in test mode, serves the generated output locally, and runs the Playwright and Axe checks. `PUBLIC_SITE_ORIGIN` must contain a root-only HTTPS origin so the Dataset Schema receives its absolute canonical `$id`.
...
The [Stage 1 Release Gate](stage-1-release-gate.md) invokes the foundation's type-check, Vitest, and Astro build boundaries. It supplies the approved production origin, directs Astro into an isolated output directory, and verifies the generated files before promotion. The foundation remains responsible for producing static pages; the gate decides whether the complete result is promotable.
