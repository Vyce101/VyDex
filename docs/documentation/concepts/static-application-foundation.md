...
The `/` route is the static [Stage 1 Homepage](stage-1-homepage.md). Current public Entry routes are generated by the [Stage 1 Entry Page](stage-1-entry-page.md), while the [Stage 1 Methodology Page](stage-1-methodology-page.md) generates `/methodology/` and the single immutable `/methodology/1.0.0/` route. The project-level `404.astro` produces the generic static `404.html`. Their complete release-selected content is generated into HTML.
...
The [Stage 1 Homepage](stage-1-homepage.md), [Stage 1 Entry Page](stage-1-entry-page.md), and [Stage 1 Methodology Page](stage-1-methodology-page.md) receive resolved release data through the Astro boundary. Their feature modules do not load files, access the environment, generate release metadata, or fetch canonical content in the browser.
